...
- **ASR:** ~~`no_repeat_ngram_size=4`~~ **does not exist on MLX** — it is a `transformers` parameter, and `mlx_whisper.transcribe` rejects it outright (found in T2.6). The shipped config is `language="ja"`, `condition_on_previous_text=False`, relying on MLX's own `compression_ratio_threshold` and temperature fallback. The looping failure (557 insertions, 70 s on one 4.9 s clip) was observed on the **transformers** path.
  - **This makes T0.3 a confounded comparison**: the kotoba candidates had the mitigation, the winner did not. The confound points against the winner, the 6.7× gap and the Stage 1 disqualifications are both far too large to be explained by it, so **the decision stands** — but the margin is not a reliable figure. See the correction in `asr.md` and standing constraint 8.
